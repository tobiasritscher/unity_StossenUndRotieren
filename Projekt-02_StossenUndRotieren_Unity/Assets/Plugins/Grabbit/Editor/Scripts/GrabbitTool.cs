...
/*                Debug.LogWarning(
        ...
        private void HandleMeshRenderers(MeshRenderer[] rens, bool displayProgress=true)
        {
            ...
            foreach (var mr in rens)
            {
                ...
                //TODO issue here: self colliding with your own kinematic RB: must introduce a hierarchical analysis :/
        ...
        private void PreparePhysicsDeactivation()
        {
            ...
            foreach (var saveState in savedDisabledRigidbodies)
            {
                ...
                saveState.Value.UnregisterRigidBody(rb);
            }
        ...
        private void PlacementModeGUI()
        {
            ...
            if (isToolSelected)
            {
                foreach (var handler in selectionHandlers)
                {
                    if (settings.showCollisionIndicators)
                    {
                        handler.DisableSelectionModeColliders(settings);
                        ...
                        handler.EnableSelectionModeColliders(settings);
                    }
                }
            }
            else
        ...
        private void UpdatePhysics()
        {
            ...
            if (!isToolSelected && settings.SetKinematicWhenNotActive)
            {
                foreach (var handler in selectionHandlers)
                {
                    if (!handler.Body)
                        continue;
                    handler.Body.collisionDetectionMode = CollisionDetectionMode.ContinuousSpeculative;
                    handler.Body.isKinematic = true;
                }
            }
        ...
        private void PrepareRigidBody(Rigidbody rb)
        {
